...
PWA への対応は [`vite-plugin-pwa`](https://vite-pwa-org.netlify.app/) によって実現されており、これは**オプションのピア依存関係**として提供されます。デフォルトではインストールされないため、`pwa` を有効にしていなければダウンロードされることはありません。

最初に `pwa` を有効にしたとき、Slidev CLI はパッケージが見つからないためインストールを促すメッセージが表示されます:
...
インストールを承認すると、Slidev はプロジェクトのパッケージマネージャー (もしくは Slidev がグローバルにあればグローバルのパッケージマネージャー) を使ってインストールします。CI のような非インタラクティブ環境ではプロンプトは表示されないので、次のようにしてインストールしてください:
